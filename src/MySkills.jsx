...
export default function MySkills({ scale, rotate, opacity, zIndex }) {
  ...
  const skillCards = [
    ...
    },
    {
      ...
      icon: "🤝",
      front: ["Team Lead", "Communication", "Agile", "Presenting"],
  ...
  return (
    <motion.section
      ...
      style={{ scale, rotate, opacity, zIndex, minHeight: 'calc(100vh - 64px)' }}
    >
      <div className="relative w-full max-w-6xl px-4 mx-auto text-center sm:px-6">
        ...
        <div className="grid grid-cols-2 gap-4 mt-12 sm:gap-5 md:grid-cols-4 md:gap-6 lg:gap-8">
          {skillCards.map((card, index) => (
            ...
            >
              <div className={`absolute inset-0 rounded-xl transition-all duration-500 transform rotate-1 group-hover:rotate-2 bg-gradient-to-br ${card.gradient} ${flippedCard === index ? 'opacity-100 scale-105' : 'opacity-90 scale-100'}`}></div>
              ...
              <div className="relative h-full overflow-hidden">
                ...
                >
                  <div className="w-full max-h-full overflow-y-auto text-center">
                    <span className="block mb-3 text-3xl sm:text-4xl">{card.icon}</span>
                    <p className="px-2 text-xs text-gray-700 sm:text-sm">{card.back}</p>
                  </div>
                ...
                >
                  ...
                  <h3 className="mb-3 text-lg font-bold text-gray-800 sm:text-xl">{card.category}</h3>
                  ...
                  <div className="pt-2 mt-auto text-[10px] sm:text-xs text-gray-400">
                    {window.innerWidth > 768 ? 'Click to learn more' : 'Tap to learn more'}
                  </div>
                </motion.div>
              </div>
            </motion.div>
